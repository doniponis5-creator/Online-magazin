...
- Обратный отсчёт акции «Специально для вас». Срок ставит владелец в 1С: «Панель сайта» →
  «Акция идёт до» (пусто — акции нет). Хранится в настройке `SITE_PROMO_UNTIL`, время
  бишкекское; сайт считает от явного `+06:00`, а не от часов устройства — покупатель может
  быть в другом поясе.
  - Отсчёт рисуется наклейкой на каждой карточке товара в этой секции, а не строкой в
    заголовке: так видно, что кончается именно это предложение.
  - Меньше часа — наклейка становится сплошной красной. Живая точка пульсирует. Всё движение
    выключается при `prefers-reduced-motion`.
  - Срок прошёл или не задан — блока нет вовсе. Врать про «осталось два часа» каждый день
    нельзя: покупатель проверяет.
