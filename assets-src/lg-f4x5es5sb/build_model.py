...
m_silver = make_mat('BodySilver', (0.80, 0.82, 0.85), 0.34, 1.0, 0.45)
m_silver_side = make_mat('BodySilverDark', (0.62, 0.65, 0.70), 0.42, 1.0, 0.35)
m_black = make_mat('PanelBlack', (0.015, 0.015, 0.02), 0.22, 0.25)
m_chrome = make_mat('Chrome', (0.90, 0.91, 0.94), 0.06, 1.0)
m_glass = make_mat('DoorGlass', (0.015, 0.02, 0.03), 0.05, 0.85)
...
m_floor = make_mat('Floor', (0.90, 0.91, 0.93), 0.65)
...
# переключатель (центр панели)
knob_z = (pz_top + pz_bot) / 2
add_cyl('Knob', 0.040, 0.016, (0.0, FRONT + 0.006, knob_z), m_chrome)
add_cyl('KnobRim', 0.047, 0.006, (0.0, FRONT + 0.002, knob_z), m_black)
# метка на переключателе
bpy.ops.mesh.primitive_cube_add(size=1, location=(0.0, FRONT + 0.0145, knob_z + 0.018))
tick = bpy.context.active_object
tick.name = 'KnobTick'
tick.dimensions = (0.006, 0.002, 0.014)
tick.data.materials.append(m_seam)

# дисплей (слева)
add_box('Display', (0.115, 0.004, 0.040), (-0.185, FRONT + 0.003, knob_z), m_display, bevel=0.002)
# кнопки (справа, ряд)
for i in range(4):
    add_cyl('Btn%d' % i, 0.008, 0.006,
            (0.13 + i * 0.038, FRONT + 0.005, knob_z), m_plastic)

# ---------- дверца ----------
add_cyl('DoorRingOuter', DOOR_R, 0.030, (0, FRONT + 0.012, DOOR_Z), m_chrome, rot=(math.pi / 2, 0, 0))
add_cyl('DoorRingInner', DOOR_R - 0.028, 0.034, (0, FRONT + 0.012, DOOR_Z), m_silver_side,
        rot=(math.pi / 2, 0, 0))
add_cyl('DoorGlass', DOOR_R - 0.036, 0.026, (0, FRONT + 0.010, DOOR_Z), m_glass,
        rot=(math.pi / 2, 0, 0))
add_cyl('DrumBehind', DOOR_R - 0.052, 0.012, (0, FRONT - 0.030, DOOR_Z), m_drum_dark,
        rot=(math.pi / 2, 0, 0))
...
# ---------- выдвижной лоток (слева, под панелью) ----------
dz = pz_bot - 0.055
add_box('Detergent', (0.165, 0.016, 0.052), (-0.175, FRONT + 0.001, dz), m_silver_side, bevel=0.004)
add_box('DeterrentSeam', (0.172, 0.004, 0.054), (-0.175, FRONT - 0.006, dz), m_seam, bevel=0.002)
add_box('DetergentHandle', (0.105, 0.012, 0.010), (-0.175, FRONT + 0.008, dz - 0.014), m_plastic, bevel=0.003)
...
add_light('Key', 'AREA', 2.6, 520, (0.9, 2.3, 2.7), (1.0, 0.98, 0.95))
add_light('Fill', 'AREA', 2.2, 150, (-2.6, 1.5, 1.7))
