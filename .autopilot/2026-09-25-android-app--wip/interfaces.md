...
- Файл для Google Play: `build-play/smarket-1.0-code3.aab` (вне git), `versionCode` 3, `versionName` 1.0, подпись — ключ загрузки `smarket-upload` (тот же сертификат, что у code2). Следующий `versionCode` — 4.
- Проверка входа без демо-кода: `server.url` временно → `http://10.0.2.2:3100` + разрешение http, локальный сайт `npx next dev -p 3100` в тестовом режиме (код 1234); всё возвращено к `https://smarket.kg`.
- Конец сессии на эмуляторе для проверки отпечатка — перевод часов эмулятора вперёд (сессия 180 дней), потом `auto_time=1`.
