...
/** Один знак «S» без подписи — для крупной подложки на фирменных блоках. */
export function BrandMark({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 55.96 100" aria-hidden="true">
  ...
  return (
    <span className="brand">
      ...
      <span className="brand__name">
        {/* Программа называется SBonus+; плюс — часть имени, поэтому он здесь,
            а не в текстах: иначе в одном месте «SBonus», в другом «SBonus+». */}
        {bonus ? (
          <>
            SBonus<span className="brand__plus">+</span>
          </>
        ) : (
          'Смарт Центр'
        )}
