...
def build_registers(out):
    build_register(
        ...
        [("Скрыть", "Скрыть с сайта", t_bool()),
         ...
         ("ДляВас", "Специально для вас", t_bool()),
         ("Хит", "Хит", t_bool()),
         ("Новинка", "Новинка", t_bool()),
         ("СтоимостьДоставки", "Стоимость доставки", t_num(15, 2)),
         ("АкцияДо", "Акция на сайте до", t_date()),
         # Гарантия: 0 — не указана. Чат на сайте называет срок только отсюда.
         ("ГарантияМесяцев", "Гарантия, месяцев", t_num(3, 0)),
...
def list_form():
    ...
    columns = [
        ...
        ("Распродажа", "Распродажа", t_bool()),
        ("ТоварДня", "Товар дня", t_bool()),
        ("ДляВас", "Для вас", t_bool()),
        ("Хит", "Хит", t_bool()),
        ("Новинка", "Новинка", t_bool()),
        ("СтоимостьДоставки", "Доставка, сом", t_num(15, 2)),
        ("АкцияДо", "Акция до", t_date()),
...
def card_form():
    ...
    for name, title, type_xml in [
        ...
        ("Новинка", "Новинка", t_bool()),
        ("СтоимостьДоставки", "Доставка, сом", t_num(15, 2)),
        ("АкцияДо", "Акция идёт до", t_date()),
        ("ГарантияМесяцев", "Гарантия, мес.", t_num(3, 0)),
    ]:
    ...
    settings = f.group("ГруппаНастройки", [
        ...
        f.check("ДляВас", "ДляВас", title_location="Right", events=change),
        f.check("Хит", "Хит", title_location="Right", events=change),
        f.check("Новинка", "Новинка", title_location="Right", events=change),
        # Срок акции у каждого товара свой: на сайте над картинкой идёт отсчёт.
        f.input("АкцияДо", "АкцияДо", width=18, events=change, hint="пусто — без отсчёта"),
        # Гарантию называет покупателю чат на сайте и в Telegram. 0 — чат скажет «уточните у сотрудника».
        f.input("ГарантияМесяцев", "ГарантияМесяцев", width=6, events=change, hint="0 — не указана"),
    ], title="На сайте")
...
def panel_form():
    ...
    f.attribute("МаксимумБонусамиСом", t_num(6, 0), title="Бонусами за один заказ — не больше, сом", saved_data=True)
    ...
    return f.render("Панель сайта", items, {"OnCreateAtServer": "ПриСозданииНаСервере"},
                    bar_buttons=[
                        f.button("ФормаОбновить", "Обновить", bar=True, default=True),
                        f.button("ФормаОткрытьСайт", "ОткрытьСайт", bar=True),
                    ])
...
FORMS = [
    ...
    ("ПанельСайта", "Панель сайта", panel_form, "ПанельСайтаМодуль.bsl"),
]
