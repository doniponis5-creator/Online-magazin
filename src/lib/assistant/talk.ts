...
export function detectLang(text: string, fallback: TalkLang): TalkLang {
  ...
  // Кыргызский, набранный без ө, ү, ң: в телефоне их часто нет, и «төлөйбүзбү»
  // пишут как «толойбузбу». Берём целые слова — «менин» есть и в узбекском
  // «менинг», поэтому кусок строки здесь не годится.
  if (/(?<![\p{L}])(менин|сиздерге|сиздер|жооп|берсениз|кылсак|келгенден|кийин|акчасын|акча|суроо|суроомо|толойбуз|толойбузбу|эмне|жакшы|качан|алабыз|кандай)(?![\p{L}])/iu.test(low)) {
    return 'ky'
  }
