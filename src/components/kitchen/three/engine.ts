...
import { buildKitchen, CEILING_LAYER, WALL_H, type BuildInput, type Built, type CabInfo, type Dims } from './build'
...
export class KitchenEngine {
  ...
  constructor(
    ...
    el.addEventListener('pointercancel', () => this.stopDrag(false))
  ...
  private onDown(e: PointerEvent) {
    ...
    else if (pick.cab?.row === 'base' && pick.dims) what = { cab: pick.cab, w: pick.dims.w }
    if (!what) return
    const target = what
  ...
  private onMove(e: PointerEvent) {
    ...
    this.hoverFrame = requestAnimationFrame(() => {
      const hit = this.hit(e)
      const p = this.pickOf(hit)
      this.renderer.domElement.style.cursor = p.slot || p.item || this.openableOf(hit?.object ?? null) ? 'pointer' : 'grab'
    })
  }
  ...
  private moveDrag(e: PointerEvent) {
    ...
    drag.target = { wall: run.id as WallId, pos: Math.round(logical * 100) }
    this.showMarker(run, x, drag.w)
  }

  private showMarker(run: { ox: number; oz: number; rot: number }, x: number, w: number) {
    if (!this.marker) {
      const g = new THREE.Group()
    ...
    g.visible = true
    g.scale.set(w, 1, 1)
    const local = new THREE.Vector3(x, 0.45, 0.31)
    g.position.copy(local.applyAxisAngle(new THREE.Vector3(0, 1, 0), run.rot)).add(new THREE.Vector3(run.ox / 100, 0, run.oz / 100))
